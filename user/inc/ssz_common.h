...
#define SSZ_COMMON_TMP_BUFF_MAX_SIZE 80
...
//CPU run hz
int32_t ssz_system_clock(void);
//system reset
void ssz_system_reset(void);
//delay microsecond
...
//sleep
void ssz_sleep(void);
void ssz_clear_watchdog(void);
//read a char from terminal
char ssz_get_char(void);

//set the last error code
void ssz_set_last_error(int error_code);
//get the last error code
int ssz_last_error(void);
...
void ssz_debug_break(void);
#else 
#define ssz_debug_break(void)
#endif

//return the now time, used at ssz_print, ssz_traceln
const char* ssz_now_time_str_for_print(void);
